...
def patch_refine(inputs_tuple, patch_rate, temp, positive_rate):
    ...
    for i in range(B):
        sort_mat, indices = torch.sort(mat[i,:])
        negatives.append(sort_mat[0, :rate])  # most dissimilar
        positives.append(sort_mat[0, -1])  # most similar
        with torch.no_grad():
            positive_index.append(indices[0, -1].item())
            negatives_index.append(indices[0, :rate].tolist())
            positive_feature = tokens[i, positive_index[i], :].unsqueeze(0)
            negative_feature = tokens[i, negatives_index[i], :]
            patch_feature.append(torch.cat((positive_feature, negative_feature), dim=0))
    ...
    patch_feature = torch.stack(patch_feature)
    ...
    return patch_out, patch_feature
...
class ClusterMemory(nn.Module, ABC):
    ...
    def forward(self, inputs_tuple, targets, indexes, feature_memory, k, patch_rate, positive_rate):
        ...
        patch_out, patch_feature = patch_refine(inputs_tuple, patch_rate, self.temp, positive_rate)
        ...
        return loss + anchor_loss + patch_loss, patch_feature
        # return loss
