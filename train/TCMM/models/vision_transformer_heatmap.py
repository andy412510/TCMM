...
class TransReID(nn.Module):
    ...
    def forward_features(self, x):
        ...
        return x[:, 0], x[:, 1], x[:, 2], x[:, 3], x[:, 5:], y

    def forward(self, x):
        cls, pt1, pt2, pt3, tokens, _ = self.forward_features(x)
...
def vit_small_heatmap(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, feat_fusion='cat', multi_neck=True, **kwargs):
    model = TransReID(
